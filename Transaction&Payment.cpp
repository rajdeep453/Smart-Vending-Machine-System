...
public:
    void createTransaction(int pid, string pname, int qty, float pr) {
        t.setTransaction(pid, pname, qty, pr);
    }
    ...
    void makePayment() {
        ...
        case 4: {
            ...
            if (cvv >= 100 && cvv <= 999) {
                ...
                rout << "Product ID     : " << t.productId << endl;
                rout << "Product Name   : " << t.productName << endl;
                rout << "Quantity       : " << t.quantity << endl;
                rout << "Price per item : " << t.price << endl;
...
int main() {
    Payment p;
    p.createTransaction(101, "Chips", 2, 10.0);
